...
class Fast_SCNN(torch.nn.Module):
    def __init__(self, input_channel, num_classes):
        ...
        self.classifierSEG = ClassifierSEG(num_classes)
        self.classifierREG = ClassifierREG()

    def forward(self, x):
        shared = self.learning_to_downsample(x)
        x = self.global_feature_extractor(shared)
        x = self.feature_fusion(shared, x)
        m = self.classifierSEG(x)
        n = self.classifierREG(x)

        m = torch.nn.functional.interpolate(m, scale_factor=8.0, mode='bilinear')
        n = torch.nn.functional.interpolate(n, scale_factor=8.0, mode='bilinear')
        return m, n
...
class FeatureFusionModule(torch.nn.Module):
    ...
    def forward(self, high_res_input, low_res_input):
        low_res_input = F.interpolate(input=low_res_input, scale_factor=4.0, mode='bilinear', align_corners=True)
...
class ClassifierSEG(torch.nn.Module):
