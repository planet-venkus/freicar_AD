...
class FreiCarLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # To Fetch RGB values
        file_path = searchForFiles(self.filenames[index] + '.png', self.db_path + '/detection_data/synth/')
        path_input = [str(p) for p in file_path]
        rgb = loadRGB(path_input, None, self.padding)

        # To Fetch REG values
        file_path = searchForFiles(self.filenames[index] + '.png', self.db_path + '/seg_reg_data/lane_dt/')
        path_input = [str(p) for p in file_path]
        reg = loadRegressionOrSegmentation(path_input, None, self.padding)

        # To Fetch SEG values
        file_path = searchForFiles(self.filenames[index] + '.png', self.db_path + '/seg_reg_data/segmentation_labels/')
        path_input = [str(p) for p in file_path]
        seg = loadRegressionOrSegmentation(path_input, None, self.padding)

        out = {'rgb': rgb[0], 'reg': reg[0], 'seg': seg[0]}
        ######################################
        # TODO: Implement me!
        ######################################
